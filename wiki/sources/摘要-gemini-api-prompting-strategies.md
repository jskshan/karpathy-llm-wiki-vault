...
**關鍵要點：**
- **輸入類型**：問題輸入、任務輸入、實體輸入、補全輸入
- **少樣本提示**：始終包含 3-5 個示例，確保格式一致性
- **Gemini 3 策略**：用詞精確直接、使用一致結構、定義參數、控制輸出詳細程度
- **智能體工作流程**：邏輯分解、問題診斷、風險評估、執行與恢復

**最佳實踐**：
1. 長上下文結構：先提供所有上下文，具體說明放在最後
2. 使用 XML 風格標籤（`<context>`, `<task>`）分隔內容
3. 溫度設為 1.0，降低會導致推理任務性能下降和迴圈
4. 現代推理模型自動生成「思考」文字，無需強制 Chain-of-Thought
