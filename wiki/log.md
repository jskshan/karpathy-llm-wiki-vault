...
## [2026-04-12] sync | 全庫簡體轉繁體翻譯
- **變更**: 將 wiki/ 下全部 19 個 .md 檔案、CLAUDE.md、README.md 共 21 個檔案從簡體中文翻譯為繁體中文
- **驗證**: 掃描確認無簡體專屬字元殘留（唯一例外：raw/ 原始檔案名稱 `提示设计策略  _  Gemini API.md`，屬不可變層，依規範保留）
- **CLAUDE.md 語言指令**: 已同步更新為「繁體中文」
...
**重要發現：**
1. **提示長度悖論**：研究表明 150-300 詞是最佳長度，超過 3,000 tokens 性能顯著下降
...
5. **Context Engineering 範式**：業界正在從單個提示最佳化轉向系統上下文管理
...
### 歸檔操作
所有 7 個源文件已移動至 `raw/09-archive/` 目錄。
...
- **引用**: [[5C_Framework]]
- **輸出**: 已建立 synthesis 文件 [[5c-prompt-markdown-note-taking]]
...
### ⚠️ 黃燈項
- **8 個未同步索引（文件不存在但 index.md 已註冊）**：
  - [[Zero_Shot_Prompting]] — 計畫中，待建立
  - [[APE_Framework]] — 計畫中，待建立
  - [[CO-STAR_Framework]] — 計畫中，待建立
  - [[RISEN_Framework]] — 計畫中，待建立
  - [[CRAFT_Framework]] — 計畫中，待建立
  - [[POWER_Framework]] — 計畫中，待建立
  - [[ReAct]] — 計畫中，待建立
  - [[Tree_of_Thoughts]] — 計畫中，待建立
  - [[RAG]] — 計畫中，待建立
...
### ❌ 紅燈項
- **1 個待解決的知識衝突**：
  - [[5C_Framework]] 頁面中記錄的「5C vs 複雜 DSL 之爭」（概念性標注，待補充具體衝突內容）

### 🛠️ 下一步行動
1. **高優先級**：建立核心概念頁面（Zero_Shot_Prompting、ReAct、RAG）
2. **中優先級**：建立框架頁面（APE、CO-STAR、RISEN、CRAFT、POWER）
3. **低優先級**：補充輔助概念（Constitutional_AI、DeepMind、Token_Efficiency 等）
4. **可選**：完善 5C_Framework 的知識衝突區塊，補充具體爭議點

### 統計
- 總文件數：19 個（不含 index/log）
- 存在頁面：19 個
- 孤兒頁面：0 個
- 未同步索引：8 個（計畫中的框架/技術）
- 死鏈：10 個（輔助/補充概念）
