...
提示工程（Prompt Engineering）是設計和最佳化輸入給大型語言模型（LLM）的文字提示，以引導模型生成準確、有用、符合預期輸出的技術學科。它是人機互動與 NLP 的交叉領域，涉及清晰表達意圖、提供上下文、設定約束和示例選擇。

> "Prompt 工程是迭代過程。不充分的提示可能導致模糊、不準確的響應，阻礙模型提供有意義的輸出。" —— Google 白皮書

## 核心要素

根據 Anthropic、OpenAI 和 Google 的共識，有效提示包含七個核心組件：

| 組件 | 作用 | 示例 |
|------|------|------|
| **Role/Persona** | 啟動領域知識 | "你是財富 500 強公司的高級資料科學家" |
| **Task Context** | 背景和受眾資訊 | "結果將呈現給董事會" |
| **Clear Instructions** | 核心請求 | "用 3 個要點總結" |
| **Sequential Steps** | 確保執行順序 | 編號步驟列表 |
| **Few-shot Examples** | 示範期望格式 | 3-5 個相關多樣化示例 |
...
2. **簡潔優先**：研究表明 **150-300 詞**是最佳長度，超過 3,000 tokens 性能顯著下降
...
| **GPT-4.1/5** | 對衝突指令敏感 | 避免矛盾指令，注意推理模型不要 CoT |
