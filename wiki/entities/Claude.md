...
Claude 是由 **Anthropic** 開發的大型語言模型家族，以其強大的長上下文處理能力和細致推理而聞名。Claude 系列模型強調有用性、無害性和誠實性（Helpful, Harmless, Honest - HHH 原則）。

## 主要版本

### Claude 4.6 系列（當前最新）
- **Claude Opus 4.6**：最強大的模型，擅長深度分析、研究和複雜編碼
- **Claude Sonnet 4.6**：平衡性能和速度，預設 effort=high
...
| Claude 4.6 | Adaptive Thinking | `effort` 參數 |
...
### 4. 上下文視窗
- **標準上下文**：200K+ tokens
- **Context Awareness**：模型能追蹤剩餘 token 預算
...
### 5. Agentic Capabilities（智能體能力）

Claude 4.6 在長期推理和狀態追蹤方面表現卓越：
...
3. **XML Structuring**：使用 XML 標籤分隔不同組件
4. **Role Assignment**：透過系統提示設置角色
...
- 透過原則（Constitution）而非僅人類標籤指導行為
- 目標是更有幫助、更無害、更誠實

### 自主性與安全的平衡
```
考慮你行為的可逆性和潛在影響。
鼓勵採取本地、可逆的行動（如編輯文件、執行測試），
