...
- **Gemini 3 Flash**: 輕量級版本，平衡性能與效率
- **特點**：
  - 專為高階推理和指令遵循設計
  - 自動生成內部「思考」文字
  - **不再需要複雜的提示工程**（如強制 CoT）
  - 支援 `thinking_level` 參數控制推理深度
...
- 複雜的提示工程技術（如 elaborate CoT）可能**降低**性能
...
**關鍵發現**：
- **推薦溫度：1.0**
- 降低溫度可能導致推理任務中**出現迴圈**和性能下降
- 這與傳統認知（高確定性任務用低溫）相反

### 4. 上下文視窗
- **標準上下文**：128K tokens
- **擴展上下文**：最高可達 2M tokens（適用於影片和深度文件）
